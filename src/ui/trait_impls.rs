...
impl TabWidget for crate::function::ModelPickerState {
    ...
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        ...
        if self.fetching {
            Paragraph::new(Line::from(Span::styled("[loading...]", Theme::status_warn()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if let Some(err) = &self.fetch_error {
            Paragraph::new(Line::from(Span::styled(err.clone(), Theme::status_fail()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if self.models.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no models - press Ctrl+R to fetch]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if self.filtered.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no matches - Ctrl+M to enter a manual model id]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else {
            let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, self.filtered.len());
            for row in range {
                ...
                if is_cursor { spans.push(Span::styled("> ", Theme::bold())); } else { spans.push(Span::raw("  ")); }
                spans.push(Span::raw(model.display.clone()));
                if model.display != model.id { spans.push(Span::styled("  ", Theme::dim())); spans.push(Span::styled(model.id.clone(), Theme::dim())); }
                if let Some(ctx) = model.context_window_tokens { spans.push(Span::raw("  ")); spans.push(Span::styled(format!("ctx:{}k", ctx / 1000), Theme::dim())); }
                else if model.context_needs_pick { spans.push(Span::raw("  ")); spans.push(Span::styled("[set context]", Theme::status_warn())); }
        ...
        if let Some(ref cp) = self.context_pick {
            ...
            picker_lines.push(Line::from(Span::styled(format!(" Context window for: {}", model.id), Theme::bold())));
            picker_lines.push(Line::from(""));
            let picker_cursor = if cp.focus == crate::function::ContextPickerFocus::Options { cp.cursor } else { usize::MAX };
            for (i, opt) in cp.options.iter().enumerate() {
                let prefix = if i == picker_cursor { "> " } else { "  " };
                let mods_str = if opt.modalities.is_empty() { String::new() } else { format!(" ({})", opt.modalities.join("+")) };
                let label = format!("{}{}k{}", prefix, opt.context / 1000, mods_str);
                let style = if i == picker_cursor { Theme::bold() } else { Theme::base() };
                picker_lines.push(Line::from(Span::styled(label, style)));
            }
            picker_lines.push(Line::from(""));
            let custom_prefix = if cp.focus == crate::function::ContextPickerFocus::CustomInput { "> " } else { "  " };
            let custom_label = if cp.custom_input.is_empty() { format!("{}Custom: [____]", custom_prefix) } else { format!("{}Custom: [{}]", custom_prefix, cp.custom_input) };
            let custom_style = if cp.focus == crate::function::ContextPickerFocus::CustomInput { Theme::bold() } else { Theme::base() };
            picker_lines.push(Line::from(Span::styled(custom_label, custom_style)));
            Paragraph::new(picker_lines).wrap(Wrap { trim: false }).render(area, buf);
        }
        None
    }
}
...
impl TabWidget for crate::function::ProviderPickerState {
    fn title(&self) -> &str { "provider" }
    fn hint(&self) -> &str { " Enter: pick | Up/Down: nav | type to filter | Ctrl+E: edit | Esc: close " }
    fn has_search(&self) -> bool { true }
    fn content_height(&self, _ctx: &TabCtx) -> usize {
        if self.entries.is_empty() || self.filtered.is_empty() { 1 } else { self.filtered.len() }
    }
    fn render_search(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        crate::ui::picker_widget::render_search_row(area, buf, &self.query, self.focus, false)
    }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        ...
        if self.entries.is_empty() {
            Paragraph::new(Line::from(Span::styled("  [no providers configured - open /settings]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if self.filtered.is_empty() {
            Paragraph::new(Line::from(Span::styled("  [no matches]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else {
            let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, self.filtered.len());
            for row in range {
                ...
                let y = area.y + (row - self.scroll) as u16;
                let mut spans: Vec<Span<'static>> = Vec::new();
                if is_cursor { spans.push(Span::styled("> ", Theme::bold())); spans.push(Span::raw(entry.display.clone())); }
                else { spans.push(Span::raw("  ")); spans.push(Span::raw(entry.display.clone())); }
                if is_active { spans.push(Span::raw("  ")); spans.push(Span::styled("[active]", Theme::status_ok())); }
                buf.set_line(area.x, y, &Line::from(spans), area.width);
            }
        }
        None
...
impl TabWidget for crate::function::ThinkingPickerState {
    fn title(&self) -> &str { "thinking" }
    fn has_search(&self) -> bool { true }
    fn content_height(&self, _ctx: &TabCtx) -> usize {
        if self.filtered.is_empty() { 1 } else { self.filtered.len() }
    }
    fn render_search(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        crate::ui::picker_widget::render_search_row(area, buf, &self.query, crate::function::PickerFocus::Search, false)
    }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        if self.filtered.is_empty() {
            ...
            for row in range {
                ...
                let is_cursor = row == self.cursor;
                let y = area.y + (row - self.scroll) as u16;
                let line = if is_cursor { Line::from(vec![Span::styled("> ", Theme::bold()), Span::raw(level.to_string())]) } else { Line::from(Span::raw(format!("  {level}"))) };
...
impl TabWidget for crate::function::TimelinePickerState {
    fn title(&self) -> &str { "timeline" }
    fn hint(&self) -> &str { " Enter: jump to message | Up/Down: nav | Ctrl+E: edit | Esc: close " }
    fn has_search(&self) -> bool { true }
    fn content_height(&self, _ctx: &TabCtx) -> usize {
        if self.entries.is_empty() || self.filtered.is_empty() { 1 } else { self.filtered.len() }
    }
    ...
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        if self.entries.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no messages in session]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if self.filtered.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no matches]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else {
            let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, self.filtered.len());
            for row in range {
                ...
                let is_cursor = row == self.cursor;
                let y = area.y + (row - self.scroll) as u16;
                let tag = if entry.tool_idx.is_some() { "tool" } else {
                    match entry.role {
                ...
                let tag_span = Span::styled(format!("{tag} "), if entry.tool_idx.is_some() { Theme::dim().add_modifier(Modifier::ITALIC) } else { Theme::dim() });
                if is_cursor {
                    let line = Line::from(vec![Span::styled("> ", Theme::bold()), tag_span, Span::raw(entry.preview.clone())]);
                    buf.set_line(area.x, y, &line, area.width);
                } else {
                    let line = Line::from(vec![Span::raw("  "), tag_span, Span::raw(entry.preview.clone())]);
                    buf.set_line(area.x, y, &line, area.width);
                }
            }
        }
...
impl TabWidget for crate::function::SessionPickerState {
    ...
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        if self.entries.is_empty() {
            Paragraph::new(Line::from(Span::styled(format!("[no {} sessions]", self.scope.label()), Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else if self.filtered.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no matches]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else {
            let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, self.filtered.len());
    ...
    fn render_hint(&self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) {
        ...
        let hint = Line::from(vec![Span::styled(format!(" [{}] ", self.scope.label()), Theme::bold()), Span::styled(hint_text, Theme::dim())]);
        Paragraph::new(hint).render(area, buf);
    }
}

impl TabWidget for crate::function::SessionRenameState {
    fn title(&self) -> &str { "rename" }
    fn hint(&self) -> &str { " Enter: save | Ctrl+E: edit | Esc: close " }
    fn content_height(&self, _ctx: &TabCtx) -> usize { 1 }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        let line = Line::from(vec![Span::styled(" title: ", Theme::bold()), Span::raw(self.title.clone())]);
...
impl TabWidget for crate::function::PlanState {
    fn title(&self) -> &str { "plan" }
    fn hint(&self) -> &str { " Enter: approve | R: reject | S: save | Esc: close " }
    fn content_height(&self, _ctx: &TabCtx) -> usize { 3 }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        let status = match self.approved { Some(true) => " [approved]", Some(false) => " [rejected]", None => " [pending]" };
        buf.set_line(area.x, area.y, &Line::from(vec![Span::styled("Plan: ", Theme::bold()), Span::raw(self.title.clone()), Span::styled(status, Theme::dim())]), area.width);
        if area.height >= 2 {
            let saved_line = if self.dirty { Line::from(vec![Span::styled("not saved \u{2014} press S to save", Theme::dim())]) }
            else if let Some(p) = &self.path { Line::from(vec![Span::styled("saved: ", Theme::dim()), Span::raw(p.display().to_string())]) }
            else { Line::from(Span::styled("not saved", Theme::dim())) };
            buf.set_line(area.x, area.y + 1, &saved_line, area.width);
        }
        if area.height >= 3 {
            Paragraph::new(Line::from(Span::styled("Read the plan in the session. Use the keys below to act on it.", Theme::dim()))).wrap(Wrap { trim: false }).render(Rect { y: area.y + 2, ..area }, buf);
        }
        None
    }
}
...
impl TabWidget for crate::function::AskState {
    ...
    fn content_height(&self, _ctx: &TabCtx) -> usize {
        ...
        match self.phase {
            AskPhase::Asking => {
                ...
                self.items.get(active).map(|it| 1 + it.options.len() + 1).unwrap_or(1)
            }
    ...
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        ...
        for (i, it) in self.items.iter().enumerate() {
            ...
            } else if answered { (Theme::dim(), "\u{2713}") }
            else { (Theme::base(), " ") };
            tab_spans.push(Span::styled(format!(" {mark}{label} "), style));
        }
        if total > 0 && (self.all_answered() || self.phase == AskPhase::Reviewing) {
            let style = if self.phase == AskPhase::Reviewing { Theme::underlined().add_modifier(ratatui::style::Modifier::BOLD) } else { Theme::dim() };
            tab_spans.push(Span::styled(" confirm ", style));
        }
        if !tab_spans.is_empty() { buf.set_line(area.x, area.y, &Line::from(tab_spans), area.width); }
    ...
    fn render_hint(&self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) {
        ...
        let hint = match self.phase {
            AskPhase::Asking => {
                if self.items.len() > 1 { " \u{2191}/\u{2193}: navigate | \u{2190}/\u{2192}: switch | Enter: pick | Esc: cancel " }
                else { " \u{2191}/\u{2193}: navigate | Enter: pick | Esc: cancel " }
            }
...
impl TabWidget for crate::function::TodoTabState {
    fn title(&self) -> &str { "todo" }
    fn hint(&self) -> &str { " " }
    fn content_height(&self, ctx: &TabCtx) -> usize { ctx.todos.len().max(1) }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        let todos = ctx.todos;
        if todos.is_empty() {
            Paragraph::new(Line::from(Span::styled("  [no tasks]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
        } else {
            ensure_cursor_visible(self.cursor, &mut self.scroll, area.height as usize);
            let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, todos.len());
            for row in range {
                let item = &todos[row];
                let y = area.y + (row - self.scroll) as u16;
                ...
                let status_label = match item.status.as_str() {
                    "pending" => "\u{25CB}", "in_progress" => "\u{25CC}", "completed" => "\u{2713}", _ => "?",
                };
...
impl TabWidget for crate::function::PastePreviewState {
    fn title(&self) -> &str { "paste" }
    fn hint(&self) -> &str { " Enter: paste | Esc: cancel " }
    fn content_height(&self, _ctx: &TabCtx) -> usize {
        if self.image.is_some() { 2 }
        else if let Some(ref text) = self.text { text.lines().count().min(5) }
        else { 1 }
    }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if area.height < 1 { return None; }
        ...
        if let Some(ref image) = self.image {
            ...
            let dim = if image.width > 0 && image.height > 0 { format!("{}x{} ", image.width, image.height) } else { String::new() };
            lines.push(Line::from(Span::styled(format!("image {} {dim}\u{00B7} {size_kb}KB", image.media_type), Theme::bold())));
            lines.push(Line::from(Span::styled(image.asset_path.display().to_string(), Theme::dim())));
    ...
    fn render_hint(&self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) {
        let hint_text = if let Some(ref text) = self.text {
            ...
            if overflow > 0 { format!(" ... ({overflow} more lines, {} chars)   Enter: paste | Esc: cancel ", text.len()) }
            else { format!(" {} chars   Enter: paste | Esc: cancel ", text.len()) }
        } else { String::from(" Enter: paste | Esc: cancel ") };
        Paragraph::new(Line::from(Span::styled(hint_text, Theme::dim()))).render(area, buf);
    }
}

impl TabWidget for crate::function::CompletionState {
    fn title(&self) -> &str { "completion" }
    fn content_height(&self, _ctx: &TabCtx) -> usize { self.candidates.len() }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        if self.candidates.is_empty() {
            Paragraph::new(Line::from(Span::styled("[no completion]", Theme::dim()))).wrap(Wrap { trim: false }).render(area, buf);
            return None;
        }
        let range = visible_window(self.cursor, &mut self.scroll, area.height as usize, self.candidates.len());
        for row in range {
            let c = &self.candidates[row];
            let is_cursor = row == self.cursor;
            let y = area.y + (row - self.scroll) as u16;
            let line = if is_cursor { Line::from(vec![Span::styled("> ", Theme::bold()), Span::raw(c.clone())]) } else { Line::from(Span::raw(format!("  {c}"))) };
            buf.set_line(area.x, y, &line, area.width);
        }
        None
    }
...
impl TabWidget for HotkeyTab {
    fn title(&self) -> &str { "hotkey" }
    fn content_height(&self, _ctx: &TabCtx) -> usize { 17 }
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, _ctx: &TabCtx) -> Option<(u16, u16)> {
        ...
        let lines: Vec<Line> = rows.into_iter().map(|(k, v)| {
            Line::from(vec![Span::styled(format!("{k:<14}"), Theme::bold()), Span::raw("  "), Span::raw(v)])
        }).collect();
        Paragraph::new(lines).wrap(Wrap { trim: false }).render(area, buf);
        None
    }
    ...
    fn render_body(&mut self, area: Rect, buf: &mut Buffer, ctx: &TabCtx) -> Option<(u16, u16)> {
        ...
        match &self.level {
            SettingsLevel::TopLevel => {
                ...
                body_lines.push(list_item(3 == self.cursor, "enter behavior", Some(cfg.enter_behavior.as_str().to_string())));
                body_lines.push(list_item(4 == self.cursor, "border type", Some(cfg.border_type.as_str().to_string())));
                body_lines.push(list_item(5 == self.cursor, "theme", Some(cfg.theme.as_str().to_string())));
                body_lines.push(list_item(6 == self.cursor, "auto compact", Some(if cfg.auto_compact { "on".to_string() } else { "off".to_string() })));
                body_lines.push(list_item(7 == self.cursor, "tool preview lines", Some(format!("{}", cfg.tool_preview_lines.clamp(crate::config::TOOL_PREVIEW_LINES_MIN, crate::config::TOOL_PREVIEW_LINES_MAX)))));
            }
            SettingsLevel::ProviderList => {
                ...
                for (i, id) in keys.iter().enumerate() {
                    ...
                    let name = cfg.entry(id).and_then(|e| if e.name.trim().is_empty() { None } else { Some(e.name.clone()) });
            ...
            SettingsLevel::ConfigForm(form) => {
                ...
                for f in fields.iter() {
                    ...
                    let value: Option<String> = match f {
                        ...
                        ConfigField::Key => { if !form.key_modified && !form.api_key.is_empty() { Some("(set, hidden)".to_string()) } else { Some(form.api_key.clone()) } }
                ...
                if let Some(err) = &form.form_error {
                    ...
                    body_lines.push(Line::from(Span::styled(format!("[!] {err}"), Theme::status_fail())));
                }
            }
            SettingsLevel::ThinkingDisplayList => {
                ...
                for (i, mode) in modes.iter().enumerate() {
                    ...
                    if is_current { label.push_str("  [current]"); }
                    body_lines.push(list_item(self.cursor == i, &label, None));
                }
            }
            SettingsLevel::ToolResultDisplayList => {
                use crate::config::ToolResultDisplay;
                let modes = [ToolResultDisplay::Show, ToolResultDisplay::Hide, ToolResultDisplay::ShowWhileStreaming];
                for (i, mode) in modes.iter().enumerate() {
            ...
            SettingsLevel::ToolPreviewLines => {
                use crate::config::{default_tool_preview_lines, TOOL_PREVIEW_LINES_MAX, TOOL_PREVIEW_LINES_MIN};
                let cur = cfg.tool_preview_lines.clamp(TOOL_PREVIEW_LINES_MIN, TOOL_PREVIEW_LINES_MAX);
        ...
        if let Some(err) = &self.load_error {
            body_lines.push(Line::from(""));
            body_lines.push(Line::from(Span::styled(format!("[config error] {err}"), Theme::status_fail())));
        }
        let total = body_lines.len();
        if total > 0 {
