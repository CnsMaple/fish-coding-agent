...
会话标题（显示在状态栏）初始为目录名，随后会自动从首条提示生成，并在你的首条回复后进一步优化，通常已足够。因此 `update_title` **不是强制要求**——若自动标题已能反映核心意图，直接跳过即可。仅在自动标题不贴切、或你希望更贴切的标题时，用 `update_title` 提供一个有意义、简洁的标题（≤40 字符，中文优先）。之后仅在当前主题产生实质差异时再更新。
