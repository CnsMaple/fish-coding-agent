...
pub struct StatusBar {
    ...
    /// `update_title` tool or the reply fallback). Only these titles
    /// are rendered in the input area; directory-name / prompt-derived
...
impl StatusBar {
    ...
    pub fn render_line_with_mode(&self, mode: &str) -> Line<'static> {
        ...
        if self.title_ai_generated && !self.session_title.is_empty() {
            spans.push(Span::raw(" | "));
